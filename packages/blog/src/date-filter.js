...
export default function DateFilter() {
    ...
    return (
        <FilterWrapper>
        <Archive>
            <div onClick={() => toggleDateFilter(!dateFilter)}>
                Archive
            </div>
        </Archive>
        <FilterOptions dateFilter={dateFilter}>
            <Years>
                <Arrow css={css`text-align: left`}>&lt;</Arrow>
                {Array.from(Array(7), (x, index) => startYear + index).map(item => (
                    <Year>{item}</Year>
                ))}
                <Arrow css={css`text-align: right`}>&gt;</Arrow>
            </Years>
            <Months>
                {Array.from(Array(12), (x, index) => 0 + index).map(item => (
